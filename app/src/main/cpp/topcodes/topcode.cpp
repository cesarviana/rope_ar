...
namespace TopCodes {
    ...
    void Scanner::threshold() {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                red = green = blue = image->ucdata[k];
                gData[k] = (red << 16) | (green << 8) | (blue);
                ++k;
            }
        }
        ...
        for (int j = 0; j < height; j++) {
            ...
            for (int i = 0; i < width; i++) {
                ...
                //----------------------------------------
                if (k >= width) {
                    threshold = (sum + (gData[k - width] & 0xffffff)) / (2 * s);
                } else {
                    threshold = sum / s;
                }
                ...
                //----------------------------------------
                gData[k] = (alpha << 24) + (sum & 0xffffff);

                switch (level) {

                    // On a white region. No black pixels yet
                    case 0:
                        if (alpha == 0) {  // First black encountered
                            level = 1;
                            b1 = 1;
                            w1 = 0;
                            b2 = 0;
